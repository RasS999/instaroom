...
const sendEmail = async (recipient, subject, message) => {
    try {
        const serverIP = "192.168.208.80"; // Siguraduhin na tama ang IP
        const response = await fetch(`http://${serverIP}:8000/booking_send_email.php`, {
            method: 'POST',
...
const checkOutReminder = async () => {
    try {
        ...
        snapshot.forEach(async (childSnapshot) => {
            ...
            const checkOutTime = new Date(`${booking.check_out_date} ${booking.check_out_time}`);
            const oneHourBefore = new Date(checkOutTime.getTime() - 60 * 60 * 1000);

            console.log(`Checking reminder for: ${booking.full_name} - Check-out at: ${checkOutTime}`);

            if (booking.booking_status === 'Checked-in' && now >= oneHourBefore && now < checkOutTime) {
                await sendEmail(
                    booking.email,
                    'Check-out Reminder',
                    `Dear ${booking.full_name},\n\nYour check-out time is at ${booking.check_out_time} on ${booking.check_out_date}.\nPlease proceed with check-out at the front desk.\n\nThank you!`
                );
            }
        });
    } catch (error) {
        console.error('Error sending check-out reminders:', error.message);
    }
};
...
const autoCheckOut = async () => {
    try {
        ...
        snapshot.forEach((childSnapshot) => {
            const booking = childSnapshot.val();
            const booking_trans_ref = childSnapshot.key;
            const roomRef = ref(db, `Rooms/${booking.room_number}`);
            const checkOutTime = new Date(`${booking.check_out_date} ${booking.check_out_time}`);

            if (booking.booking_status === 'Checked-in' && now >= checkOutTime) {
                console.log(`Auto-checking out: ${booking.full_name}`);

                updates.push(
                    update(ref(db, `Bookings/${booking_trans_ref}`), { booking_status: 'Checked-out' }),
                    update(roomRef, {
                        booking_trans_ref: '',
                        booking_status: '',
        ...
        console.error('Error auto-checking out guests:', error.message);
    }
};
...
const autoCancelPendingBookings = async () => {
    try {
        ...
        snapshot.forEach((childSnapshot) => {
            const booking = childSnapshot.val();
            const booking_trans_ref = childSnapshot.key;
            const checkOutDateTime = new Date(`${booking.check_out_date} ${booking.check_out_time}`);

            // Check if the booking is Pending and the check-out date and time has passed
            if (now >= checkOutDateTime && (booking.booking_status === 'Pending' || booking.payment_status === 'Pending')) {
                console.log(`Auto-cancelling: ${booking.full_name}`);

                updates.push(
                    update(ref(db, `Bookings/${booking_trans_ref}`), { 
                    ...
                    sendEmail(
                        booking.email,
                        'Booking Cancelled',
                        `Dear ${booking.full_name},\n\nYour booking has been cancelled due to non-payment or pending status.\nWe apologize for any inconvenience.\n\nBest regards,\nInstaRoom Team`
                    )
                );
            }
        });
        ...
        console.error('Error auto-cancelling pending bookings:', error.message);
    }
};
...
setInterval(() => {
    checkOutReminder();
    autoCheckOut();
    autoCancelPendingBookings();  // Add this function call here to check for cancellations
...
const populateBookingTable = async () => {
    ...
    bookings.forEach((booking) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <select class="form-control" onchange="updatePaymentStatus('${booking.booking_trans_ref}', this.value)">
            ...
            <td>
                <select class="form-control" onchange="updateBookingStatus('${booking.booking_trans_ref}', this.value, '${booking.room_number}')">
                    <option value="Pending" ${booking.booking_status === 'Pending' ? 'selected' : ''}>Pending</option>
                    <option value="Confirmed" ${booking.booking_status === 'Confirmed' ? 'selected' : ''}>Confirmed</option>
                    <option value="Checked-in" ${booking.booking_status === 'Checked-in' ? 'selected' : ''}>Checked-in</option>
